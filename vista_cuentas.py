...
class index:
    ...
    def __init__ (self,id_usuario,password):
        ...
        self.crear_cuentas1 = ttk.Button(self.ventana_cuentas,text="E",command=self.dibuja_botones)
        self.crear_cuentas1.grid(column=0,row=1)
    ...
    def dibuja_botones(self):
        ...
        self.test_var = cuenta(master_password=str(self.master_password),id_usuario=str(self.id_usuario)).verificar_cuentas()
        for row in self.test_var:
            sal = row[4]
            self.cifrado = Cifrado(self.master_password)
            cifrado_pass = self.cifrado.verificar_cifrado(sal)
            self.data_cuenta.append(self.cifrado.descifrado_suave(row[0],row[1],row[2],row[3],cifrado_pass))
        self.mensaje_exito = messagebox.showinfo(title="resultado", message=self.data_cuenta)

    def dibuja_creacuentas(self):
        self.cuenta = Creacioncuentas(self.ventana_cuentas, self.id_usuario, self.master_password)
        return self.cuenta

class Creacioncuentas:    
    def __init__(self,ventanaprincipal,id_usuario,password):
        """
    ...
    def verifica_contra(self):
        ...
        if registra_pwwd == confirma_pwwd:
            self.cuenta_nueva = cuenta(self.id_usuario,registra_sitio,registra_url,registra_login,registra_pwwd,self.master_password).crear_cuenta()
            self.mensaje_exito = messagebox.showinfo(title="resultado", message="¡Registro realizado con éxito! "+self.cuenta_nueva,parent=self.ventana_crear_cuentas)
